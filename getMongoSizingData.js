  ...
  try {
    if (dbs.databases) {
      ...
      dbs.databases.forEach(function (mydb) {
        if (!/^(admin|local|config)$/.test(mydb.name)) {
          ...
          db.getSiblingDB(mydb.name)
            .getCollectionInfos({ type: "timeseries" })
            .forEach(function (collectionInfo) {
              //check max collections;
              if (collections_counter < _maxCollections) {
                var colstats = db.getSiblingDB(mydb.name).runCommand({
                    aggregate: collectionInfo["name"],
                    pipeline: [
                ...
                collectionstats.push({
                  ...
                  bucketCount: storageStats.timeseries.bucketCount,
                  avgBucketSize: storageStats.timeseries.avgBucketSize,
...
if (_printJSON) print(JSON.stringify(_output, jsonStringifyReplacer, 0));